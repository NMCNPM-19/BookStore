...
class AccountController {
    ...
    async list(req, res, next) {
        if (req.user) {
            if (req.user.LOAINV = "adm") {
                ...
                const TotalPage =
                    Math.ceil(accounts.count / itemPerPage) > page + 1
                        ? Math.ceil(accounts.count / itemPerPage)
                        : page + 1;
    ...
    async add(req, res, next) {
        try {
            ...
            return res.render("accounts/editAccount", {
        ...
        if (req.user) {
            ...
            const page =
                !isNaN(req.query.page) && req.query.page > 0
                    ? req.query.page - 1
                    : 0;
            const accounts = await accountService.binList(
                title,
                page,
                itemPerPage
            );
